...
$faq = [
   ...
   // ],
   // [
   //    'request' => "Stabilire il paese associato al tuo account",
   ...
   [
      ...
      Tieni presente che i servizi Google sono fondamentalmente gli stessi a prescindere dalla società consociata che li offre o dal paese a cui è associato il tuo account." => [ false => [false] ] ]
   ],
   [
      'request' => "Perché il mio account è associato a un paese?",
      'answer' => [
         "Il tuo account è associato a un paese (o territorio) nei Termini di servizio per poter stabilire due cose:" => [
            "Google offre i propri servizi per i consumatori tramite una delle due società seguenti" => [
               "Google Ireland Limited, se gli utenti sono residenti nello Spazio economico europeo (paesi dell'Unione europea, oltre a Islanda, Liechtenstein e Norvegia) o in Svizzera.",
               "Google LLC, con sede negli Stati Uniti, per il resto del mondo."
            ],
            "La versione dei termini che regola il nostro rapporto, che può variare in base alle leggi locali."
   ...
   <main>
      <div class="box-faq">
         <?php foreach ($faq as $element) { ?>
            <h1><?php echo $element['request'] ?></h1>
            <?php
            foreach ($element['answer'] as $paragrafo) { ?>
               <p><?php echo '<br>' . $paragrafo ?></p>
            <?php } ?>
         <?php } ?>
      </div>
   </main>
